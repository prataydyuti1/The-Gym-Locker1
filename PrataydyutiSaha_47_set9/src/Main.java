package src;
import java.sql.*;
import java.util.Scanner;

public class Main {

    static Scanner sc = new Scanner(System.in);

    public static void main(String[] args) {

        while (true) {
            System.out.println("\n===== Gym Locker System =====");
            System.out.println("1. Add Member");
            System.out.println("2. Assign Locker");
            System.out.println("3. Release Locker");
            System.out.println("4. View Locker Status");
            System.out.println("5. Exit");
            System.out.print("Enter choice: ");

            int choice = sc.nextInt();
            sc.nextLine(); // consume newline

            switch (choice) {
                case 1: addMember(); break;
                case 2: assignLocker(); break;
                case 3: releaseLocker(); break;
                case 4: viewLockers(); break;
                case 5: System.exit(0);
                default: System.out.println("Invalid choice!");
            }
        }
    }

    // 1. Add Member
    static void addMember() {
        try (Connection conn = DBConnection.getConnection()) {

            System.out.print("Enter Name: ");
            String name = sc.nextLine();

            System.out.print("Enter Phone: ");
            String phone = sc.nextLine();

            System.out.print("Enter Plan (monthly/quarterly/annual): ");
            String plan = sc.nextLine();

            String sql = "INSERT INTO members(name, phone, plan) VALUES (?, ?, ?)";
            PreparedStatement ps = conn.prepareStatement(sql);

            ps.setString(1, name);
            ps.setString(2, phone);
            ps.setString(3, plan);

            ps.executeUpdate();

            System.out.println("✅ Member added successfully!");

        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    // 2. Assign Locker
    static void assignLocker() {
        try (Connection conn = DBConnection.getConnection()) {

            System.out.print("Enter Member ID: ");
            int memberId = sc.nextInt();

            String findLocker = "SELECT locker_id FROM lockers WHERE member_id IS NULL LIMIT 1";
            PreparedStatement ps1 = conn.prepareStatement(findLocker);
            ResultSet rs = ps1.executeQuery();

            if (rs.next()) {
                int lockerId = rs.getInt("locker_id");

                String update = "UPDATE lockers SET member_id=?, assigned_on=CURDATE() WHERE locker_id=?";
                PreparedStatement ps2 = conn.prepareStatement(update);

                ps2.setInt(1, memberId);
                ps2.setInt(2, lockerId);

                ps2.executeUpdate();

                System.out.println("✅ Locker assigned successfully!");

            } else {
                System.out.println("❌ No free lockers available!");
            }

        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    // 3. Release Locker
    static void releaseLocker() {
        try (Connection conn = DBConnection.getConnection()) {

            System.out.print("Enter Locker No: ");
            String lockerNo = sc.nextLine();

            String sql = "UPDATE lockers SET member_id=NULL, assigned_on=NULL WHERE locker_no=?";
            PreparedStatement ps = conn.prepareStatement(sql);

            ps.setString(1, lockerNo);

            int rows = ps.executeUpdate();

            if (rows > 0)
                System.out.println("✅ Locker released!");
            else
                System.out.println("❌ Locker not found!");

        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    // 4. View Lockers
    static void viewLockers() {
        try (Connection conn = DBConnection.getConnection()) {

            String sql = "SELECT l.locker_no, m.name, m.plan, l.assigned_on " +
                         "FROM lockers l LEFT JOIN members m " +
                         "ON l.member_id = m.member_id";

            PreparedStatement ps = conn.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();

            while (rs.next()) {

                String locker = rs.getString("locker_no");
                String name = rs.getString("name");

                if (name == null) {
                    System.out.println("Locker " + locker + " | [FREE]");
                } else {
                    System.out.println("Locker " + locker +
                            " | Member: " + name +
                            " | Plan: " + rs.getString("plan") +
                            " | Since: " + rs.getDate("assigned_on"));
                }
            }

        } catch (Exception e) {
            e.printStackTrace();
        }
    }
}